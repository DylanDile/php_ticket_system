...
<div>
  <div class="form-group mb-0 justify-content-end row">
    <div class="col-lg-9">
      <div v-if="! ( selected.length > 0 )" class="start mw-400">
        <div class="col-sm-6">
          Click to select {{ name }}
          <button @click.prevent="open" data-bs-target="#myModal" data-bs-toggle="modal"  class="btn btn-white btn-icon btn-sm border-0">
            <i class="mdi mdi-circle-edit-outline mdi-36px"></i>
          </button>
        </div>
                    ...
                    <div class="col-lg-8">
                      <div class="card">
                        <div class="card-body py-2 border-bottom d-flex align-items-center justify-content-between">
                          ...
                          <button @click.prevent="close" class="btn btn-white">
                        ...
                        <div v-if="( selected.length > 0 )" class="card-body py-1 bg-white text-center border-bottom d-flex align-items-center justify-content-center flex-wrap">
                          <strong class="badge badge-light font-13 p-1 m-1" v-for="item in this.selectedName">
                            <span class="text-primary">#</span> {{ item.id }} - {{ item[ select ? select : 'select_name' ]  }}
                          </strong>
                        </div>
                          ...
                          <div :key="row.id" v-for="row in data.content.data" class="border-bottom select-item">
                            <div class="px-3 py-2 d-flex align-items-center">
                              ...
                              <div class="ml-auto pointer">
                                <button @click.prevent="add(row)" v-if="selected.indexOf(row.id) === -1" class="btn btn-white btn-sm">
                                  <i class="mdi mdi-checkbox-marked-circle-outline mdi-24px"></i>
                                </button>
                                <button @click.prevent="remove(row)" v-if="selected.indexOf(row.id) !== -1" class="btn btn-icon btn-primary btn-sm shadow-none">
                        ...
                        <div class="card-body d-flex align-items-center">
                          ...
                          <div v-if="data.content.total !== data.content.to && data.content.data.length > 0" class="ml-auto text-primary pointer">
                            <button @click.prevent="data.append()" :class="['btn btn-light' , data.loading ? 'btn-loading' : '']">
...
export default {
  name: "Modal",
  props: ['value' , 'url' , 'prefix' ,'start' , 'name'  , 'select' , 'additional'],
  mounted : function()
  {
    this.data.fetch();
  },
