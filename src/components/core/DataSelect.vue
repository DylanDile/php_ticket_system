<script>
    ...
    export default {
        name: "data-select",
        props: ['value' , 'name' , 'url' , 'prefix' ,'start' , 'additional'],
        mounted : function()
        {
            this.data.fetch();
        },
                                    ...
                                    <div :key="row.id" v-for="row in data.content.data" class="border-bottom select-item">
                                        <div class="px-3 py-2 d-flex align-items-center">
                                            ...
                                            <div class="ml-auto pointer">
                                                <button @click.prevent="add(row)" v-if="row.id !== selected" class="btn btn-white btn-sm">
                                                    <i class="mdi mdi-checkbox-marked-circle-outline mdi-24px"></i>
                                                </button>
                                                <button @click.prevent="remove(row)" v-if="row.id === selected" class="btn btn-icon btn-primary btn-sm shadow-none">
